#' @title Percent spliced-in (PSI) visualization for retained introns (RI)
#'
#' @description
#' \code{PlotPSI.RI} visualizes percent spliced-in (PSI) for each genomic coordinate for retained intron (RI) event across groups of single cells.
#'
#' @details
...
#' PlotPSI.RI.MultiGroups(PSI, SampleInfo=system.file("extdata/Sample_Info",
...
PlotPSI.RI.MultiGroups <- function(object, SampleInfo, ExonInfo, statistical.test=c("KW", "ANOVA"), multiple.testing=c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"), Plots, plot.width, plot.height) {
    ...
    exon.info <- exon.info[which(exon.info$V2=="RI"), ]
    ...
    for(k in 1:nrow(exon.info)) {
        ...
        if(length(strand) != 0) {
            ...
            # Retrieve AS exon
            start.AS <- end.5.constitutive + 1
            end.AS <- start.3.constitutive - 1
     ...
     # p-value graph
        ...
        # PSI (individual) graph
            ...
            # Define column labels
                ...
                exon.AS <- rep("Intron", times=(abs(end.AS-start.AS)+1))
        ...
        # Save plots
        ggsave(paste(Plots, "//", k, "_RI_Plots_", exon.info[k, "V3"], ".pdf", sep=""), plot.final, device="pdf", width=plot.width, height=plot.height)
