#' @title Plot percent spliced-in (PSI) values for skipped-exon (SE) located on positive strand
#'
#' @description
#' \code{PlotPSI.SE.Pos} computes percent spliced-in (PSI) at each genomic coordinate for skipped-exon (SE) located on positive (forward) strand.
#'
#' @details
...
PlotPSI.SE.Pos <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/Users/seanwen/Documents/VALERIE/VALERIE/Dataset/Plots/SNRPN.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
