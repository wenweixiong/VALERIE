#' @title Plot percent spliced-in (PSI) values for retained-intron (RI) located on negative strand
#'
#' @description
#' \code{PlotPSI.RI.Neg} computes percent spliced-in (PSI) at each genomic coordinate for retained-intron (RI) located on negative (reverse) strand.
#'
#' @details
#' This function computes percent spliced-in (PSI) at each genomic coordinate for retained-intron (RI) located on negative (reverse) strand. Formula for computing PSI is number of reads with non-N CIGAR operation divided by the total number of reads. Total number of reads is the sum of reads with non-N CIGAR operation and reads with N-CIGAR operation
#'
...
#' # DO NOT RUN
#' PlotPSI.RI.Neg(
#'  tran_id="chr7:5528719:5528564:-@chr7:5528344:5528281",
#'  Bam="/Users/BAM/",
#'  BamPheno=BamPheno,
...
PlotPSI.RI.Neg <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
        
    #tran_id <- "chr2:190977025:190976840:-@chr2:190975887:190975812"
    #Bam <- "/t1-data/project/meadlab/shared/jen/full_length03/BAM_all_batches/"
    #BamPheno <- read.table("/t1-data/project/meadlab/shared/jen/full_length03/VALERIE/SF3B1_vs_WT/STAT1_RI/BAM_PhenoData.txt", sep="\t", header=TRUE, stringsAsFactors=FALSE)
    #cell.types <- c("WT", "JAK2", "SF3B1", "JAK2_SF3B1")
    #min.coverage <- 10
    #cons.exon.cutoff <- 100
    #method <- "kw"
    #method.adj <- "fdr"
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/t1-data/project/meadlab/shared/jen/full_length03/VALERIE/SF3B1_vs_WT/STAT1_RI/Heatmap.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
    #show.mean.ci <- FALSE
    ...
    if(exon.length > cons.exon.cutoff) {
        ...
        exon.2 <- paste(chr, start, end.new, sep=":")
    ...
    # Merge
    tran_id <- paste(exon.1, exon.2, sep=":-@")
    ...
    # Specify event coordinates
        ...
        end <- as.numeric(strsplit(exons[2], split=":", fixed=TRUE)[[1]][3])
        start <- as.numeric(strsplit(exons[1], split=":", fixed=TRUE)[[1]][2])
        width <- (start - end) + 1
                                    
        # Create GRanges object
        gr <- GRanges(seqnames=chr, ranges=IRanges(start=end, width=width))
    ...
    # Specify per-base coordinates
        ...
        for(i in 1:length(exons)) {
            ...
            range.exon.list[[i]] <- seq(start, end)
            
        }
        
        # Intron
        start <- as.numeric(strsplit(exons[1], split=":", fixed=TRUE)[[1]][3]) - 1
        end <- as.numeric(strsplit(exons[2], split=":", fixed=TRUE)[[1]][2]) + 1
        range.intron <- seq(start, end)
        
        # Merge
        range.list <- list(range.exon.list[[1]], range.intron, range.exon.list[[2]])
    ...
    df.merged <- rbind.data.frame(df.1, df.2, df.3)
    ...
    df$exon.type <- factor(df$exon.type,
                           levels=c("5' Cons. exon", "Intron", "3' Cons. exon")
                           )
    ...
    df.small <- df[which(df$exon.type=="Intron"), ]
    ...
    # Define column and colors
        ...
        names(annotation.col.color) <- c("5' Cons. exon", "Intron", "3' Cons. exon")
