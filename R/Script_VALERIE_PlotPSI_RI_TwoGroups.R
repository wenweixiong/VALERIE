...
#'
#' @description
#' \code{PlotPSI.RI} visualizes percent spliced-in (PSI) for each genomic coordinate for retained intron (RI) event across two groups of single cells.
#'
#' @details
...
#' PlotPSI.RI.TwoGroups(PSI, SampleInfo=system.file("extdata/Sample_Info",
...
#'   statistical.test="t.test",  multiple.testing="none",
...
PlotPSI.RI.TwoGroups <- function(object, SampleInfo, ExonInfo, statistical.test=c("wilcox", "t.test"), multiple.testing=c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"), Plots, plot.width, plot.height) {
