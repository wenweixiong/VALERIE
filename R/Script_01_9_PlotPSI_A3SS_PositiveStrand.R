#' @title Plot percent spliced-in (PSI) values for alternative 3' splice site (A3SS) located on positive strand
#'
#' @description
#' \code{PlotPSI.A3SS.Pos} computes percent spliced-in (PSI) at each genomic coordinate for alternative 3' splice site (A3SS) located on positive (forward) strand.
#'
#' @details
#' This function computes percent spliced-in (PSI) at each genomic coordinate for alternative 3' splice site (A3SS) located on positive (forward) strand. Formula for computing PSI is number of reads with non-N CIGAR operation divided by the total number of reads. Total number of reads is the sum of reads with non-N CIGAR operation and reads with N-CIGAR operation
#'
...
#' # DO NOT RUN
#' PlotPSI.A3SS.Pos(
#'  tran_id="chr11:57789037:57789155
#'           :+@chr11:57791385|57791492:57791673",
#'  Bam="/Users/BAM/",
#'  BamPheno=BamPheno,
...
PlotPSI.A3SS.Pos <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/Users/seanwen/Documents/VALERIE/VALERIE/Dataset/Plots/RPS24.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
    ...
    exon.2.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
    
    # 3' constitutive exon
    . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][2]
    . <- strsplit(., split="|", fixed=TRUE)[[1]][2]
    start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1])
    ...
    if(exon.length > cons.exon.cutoff) {
        ...
        exon.3.end <- start + (cons.exon.cutoff - 1)
    ...
    # Merge
    tran_id <- paste(exon.1, ":+@", chr, ":", exon.2.start, "|",
                     exon.3.start, ":", exon.3.end, sep="")
    ...
    # Specify event coordinates
        ...
        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][1]
        starts[1] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
        
        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][1]
        ends[1] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
        
        widths[1] <- (ends[1] - starts[1]) + 1
        
        # alt + 3' constitutive
        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][2]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
        starts[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])

        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][2]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][2]
        ends[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
        
        widths[2] <- (ends[2] - starts[2]) + 1
    ...
    # Specify per-base coordinates
        ...
        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][2]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][2]
        end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1]) - 1
        
        range.list[[2]] <- seq(start, end)
    ...
    df$exon.type <- factor(df$exon.type,
                           levels=c("5' Cons. exon", "3' Alt. exon", "3' Cons. exon")
                           )
    ...
    df.small <- df[which(df$exon.type=="3' Alt. exon"), ]
    ...
    # Define column and colors
        ...
        names(annotation.col.color) <- c("5' Cons. exon", "3' Alt. exon", "3' Cons. exon")
