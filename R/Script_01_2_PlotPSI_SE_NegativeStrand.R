#' @title Plot percent spliced-in (PSI) values for skipped-exon (SE) located on negative strand
#'
#' @description
#' \code{PlotPSI.SE.Neg} computes percent spliced-in (PSI) at each genomic coordinate for skipped-exon (SE) located on negative (reverse) strand.
#'
#' @details
#' This function computes percent spliced-in (PSI) at each genomic coordinate for skipped-exon (SE) located on negative (reverse) strand. Formula for computing PSI is number of reads with non-N CIGAR operation divided by the total number of reads. Total number of reads is the sum of reads with non-N CIGAR operation and reads with N-CIGAR operation
#'
...
#' # DO NOT RUN
#' PlotPSI.SE.Neg(
#'  tran_id="chr12:110502049:110502117:-@chr12:110499535:110499546
#'           :-@chr12:110496012:110496203",
#'  Bam="/Users/BAM/",
#'  BamPheno=BamPheno,
...
PlotPSI.SE.Neg <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/Users/seanwen/Documents/VALERIE/VALERIE/Dataset/Plots/VPS29.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
    ...
    # 3' constitutive exon
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][3]
    chr <- strsplit(., split=":", fixed=TRUE)[[1]][1]
    start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
    end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
    exon.length <- (start - end) + 1
    ...
    df$exon.type <- factor(df$exon.type,
                           levels=c("5' Cons. exon", "Alt. exon", "3' Cons. exon")
                           )
    ...
    df.small <- df[which(df$exon.type=="Alt. exon"), ]
