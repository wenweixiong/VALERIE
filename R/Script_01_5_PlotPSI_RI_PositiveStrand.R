#' @title Plot percent spliced-in (PSI) values for retained-intron (RI) located on positive strand
#'
#' @description
#' \code{PlotPSI.RI.Pos} computes percent spliced-in (PSI) at each genomic coordinate for retained-intron (RI) located on positive (forward) strand.
#'
#' @details
...
PlotPSI.RI.Pos <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
    ...
    # Specify event coordinates
        ...
        gr <- GRanges(seqnames=chr, ranges=IRanges(start=start, width=width))
