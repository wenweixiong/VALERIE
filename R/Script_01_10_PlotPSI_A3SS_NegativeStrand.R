#' @title Plot percent spliced-in (PSI) values for alternative 3' splice site (A3SS) located on negative strand
#'
#' @description
#' \code{PlotPSI.A3SS.Neg} computes percent spliced-in (PSI) at each genomic coordinate for alternative 3' splice site (A3SS) located on negative (reverse) strand.
#'
#' @details
#' This function computes percent spliced-in (PSI) at each genomic coordinate for alternative 3' splice site (A3SS) located on negative (reverse) strand. Formula for computing PSI is number of reads with non-N CIGAR operation divided by the total number of reads. Total number of reads is the sum of reads with non-N CIGAR operation and reads with N-CIGAR operation
#'
...
#' # DO NOT RUN
#' PlotPSI.A3SS.Neg(
#'  tran_id="chr3:129171446:129171538
#'           :-@chr3:129171277|129171280:129171079",
#'  Bam="/Users/BAM/",
#'  BamPheno=BamPheno,
...
PlotPSI.A3SS.Neg <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE) {
    ...
    exon.2.start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1])
    ...
    # Merge
    tran_id <- paste(exon.1, ":-@", chr, ":", exon.3.start, "|",
                     exon.2.start, ":", exon.3.end, sep="")
    ...
    # Specify event coordinates
        ...
        . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][1]
        chr <- strsplit(., split=":", fixed=TRUE)[[1]][1]
        ends[1] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
        starts[1] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
        ...
        . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][2]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][2]
        starts[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][1])
    ...
    # Specify per-base coordinates
        ...
        . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][2]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
        end <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2]) + 1
