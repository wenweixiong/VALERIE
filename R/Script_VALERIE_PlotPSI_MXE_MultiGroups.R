#' @title Percent spliced-in (PSI) visualization for mutually exclusive exons (MXE)
#'
#' @description
#' \code{PlotPSI.MXE} visualizes percent spliced-in (PSI) for each genomic coordinate for mutually exclusive exons (MXE) event across groups of single cells.
#'
#' @details
...
#' @param statistical.test Method for comparising PSI values at each genomic coordinates between groups of single cells.
...
#' PlotPSI.MXE.MultiGroups(PSI, SampleInfo=system.file("extdata/Sample_Info",
...
PlotPSI.MXE.MultiGroups <- function(object, SampleInfo, ExonInfo, statistical.test=c("KW", "ANOVA"), multiple.testing=c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"), Plots, plot.width, plot.height) {
    ...
    exon.info <- exon.info[which(exon.info$V2=="MXE"), ]
    ...
    for(k in 1:nrow(exon.info)) {
        ...
        if(length(strand) != 0) {
            ...
            # Retrieve 5' AS exon
            exon.AS.5 <- exons.split[2]
            exon.AS.5 <- strsplit(exon.AS.5, split="\\:")
            start.AS.5 <- as.numeric(exon.AS.5[[1]][2])
            end.AS.5 <- as.numeric(exon.AS.5[[1]][3])
            
            # Retrieve 3' AS exon
            exon.AS.3 <- exons.split[3]
            exon.AS.3 <- strsplit(exon.AS.3, split="\\:")
            start.AS.3 <- as.numeric(exon.AS.3[[1]][2])
            end.AS.3 <- as.numeric(exon.AS.3[[1]][3])
            
            # Retrieve 3' constitutive exon
            exon.3.constitutive <- exons.split[4]
            ...
            # Retrieve coordinates
                ...
                # Subset 5' AS exon
                psi.small.exon.AS.5 <- psi.small[which(psi.small$Position >= start.AS.5 & psi.small$Position <= end.AS.5), ]
                
                # Subset 3' AS exon
                psi.small.exon.AS.3 <- psi.small[which(psi.small$Position >= start.AS.3 & psi.small$Position <= end.AS.3), ]
                ...
                # Merge
                psi.small <- rbind.data.frame(psi.small.exon.5.constitutive, psi.small.exon.AS.5, psi.small.exon.AS.3, psi.small.exon.3.constitutive)
            ...
            exon.AS.5 <- exons.split[2]
            exon.AS.5 <- strsplit(exon.AS.5, split="\\:")
            start.AS.5 <- as.numeric(exon.AS.5[[1]][3])
            end.AS.5 <- as.numeric(exon.AS.5[[1]][2])
            
            # Retrieve 3' AS exon
            exon.AS.3 <- exons.split[3]
            exon.AS.3 <- strsplit(exon.AS.3, split="\\:")
            start.AS.3 <- as.numeric(exon.AS.3[[1]][3])
            end.AS.3 <- as.numeric(exon.AS.3[[1]][2])
       ...
       # Compute mean PSI for each base
        ...
        # PSI (individual) graph
            ...
            # Reorder data frame (aesthetic purpose)
                ...
                exp.1_exon_AS <- exp.1[,which(names(exp.1) %in% c(start.AS.3:end.AS.3))]
                exp.2_exon_AS <- exp.2[,which(names(exp.2) %in% c(start.AS.3:end.AS.3))]
                exp.3_exon_AS <- exp.3[,which(names(exp.3) %in% c(start.AS.3:end.AS.3))]
            ...
            # Define column labels
                ...
                exon.AS.5 <- rep("5' Alt. exon", times=(abs(end.AS.5-start.AS.5)+1))
                exon.AS.3 <- rep("3' Alt. exon", times=(abs(end.AS.3-start.AS.3)+1))
                exon.3.constitutive <- rep("3' Cons. exon", times=(abs(end.3.constitutive-start.3.constitutive)+1))
                annotation.col.lab <- data.frame("Base position"=c(exon.5.constitutive, exon.AS.5, exon.AS.3, exon.3.constitutive))
            ...
            # Define column and colors
                # Columnm color
                annotation.col.color <- c("black", "brown", "orange", "black")
            ...
            # Plot
            p3 <- pheatmap(exp, cluster_cols=FALSE, cluster_rows=FALSE, scale="column", show_rownames=FALSE, show_colnames=FALSE, color=color, annotation_col=annotation.col.lab, annotation_row=annotation.row.lab, annotation_colors=annotation.row.col.color, silent=TRUE, border_color=NA, fontsize=10, main=exon.info[1,3])
