#' @title Plot percent spliced-in (PSI) values for alternative 5' splice site (A5SS) located on positive strand
#'
#' @description
#' \code{PlotPSI.A5SS.Pos} computes percent spliced-in (PSI) at each genomic coordinate for alternative 5' splice site (A5SS) located on positive (forward) strand.
#'
#' @details
...
PlotPSI.A5SS.Pos <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
        
    #tran_id <- "chr3:197950190:197950221|197950299:+@chr3:197950936:197950978"
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/Users/seanwen/Documents/VALERIE/VALERIE/Dataset/Plots/RPL35A.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
    ...
    . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][1]
    exon.2 <- strsplit(., split="|", fixed=TRUE)[[1]][2]
    ...
    if(exon.length > cons.exon.cutoff) {
        ...
        exon.3 <- paste(chr, start, end.new, sep=":")
    ...
    # Merge
    tran_id <- paste(exon.1, "|", exon.2, ":+@", exon.3, sep="")
    ...
    # Specify event coordinates
        ...
        ends[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
    ...
    # Specify per-base coordinates
        ...
        . <- strsplit(tran_id, split=":+@", fixed=TRUE)[[1]][1]
        . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
        start <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3]) + 1
