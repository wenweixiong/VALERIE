#' @title Plot percent spliced-in (PSI) values for alternative 5' splice site (A5SS) located on negative strand
#'
#' @description
#' \code{PlotPSI.A5SS.Neg} computes percent spliced-in (PSI) at each genomic coordinate for alternative 5' splice site (A5SS) located on negative (reverse) strand.
#'
#' @details
#' This function computes percent spliced-in (PSI) at each genomic coordinate for alternative 5' splice site (A5SS) located on negative (reverse) strand. Formula for computing PSI is number of reads with non-N CIGAR operation divided by the total number of reads. Total number of reads is the sum of reads with non-N CIGAR operation and reads with N-CIGAR operation
#'
...
#' # DO NOT RUN
#' PlotPSI.A5SS.Neg(
#'  tran_id="chr6:34426071:34425796|34426032
#'           :-@chr6:34425072:34425221",
#'  Bam="/Users/BAM/",
#'  BamPheno=BamPheno,
...
PlotPSI.A5SS.Neg <- function(tran_id, Bam, BamPheno, cell.types, min.coverage, cons.exon.cutoff, method, method.adj, sig.pval=0.10, cell.types.colors, plot.title, plot.width, plot.height, plot.out, track=TRUE, nboots=2000, show.mean.ci=TRUE) {
    ...
    #plot.width <- 5
    #plot.height <- 8
    #plot.out <- "/Users/seanwen/Documents/VALERIE/VALERIE/Dataset/Plots/RPL8.pdf"
    #track <- TRUE
    #sig.pval <- 0.10
    ...
    if(exon.length > cons.exon.cutoff) {
        ...
        exon.1.start <- end + (cons.exon.cutoff - 1)
    ...
    . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][1]
    . <- strsplit(., split="|", fixed=TRUE)[[1]][1]
    exon.2.end <- . <- strsplit(., split=":", fixed=TRUE)[[1]][3]
    ...
    if(exon.length > cons.exon.cutoff) {
        ...
        exon.3 <- paste(chr, end.new, start, sep=":")
        
    } else {
        
        exon.3 <- paste(chr, end, start, sep=":")
        
    }
    
    # Merge
    tran_id <- paste(chr, ":", exon.1.start, ":", exon.2.end, "|", exon.1.end,
                    ":-@", exon.3, sep="")
    ...
    # Specify event coordinates
        ...
        widths[1] <- (starts[1] - ends[1]) + 1
        
        # 3' constitutive
        . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][2]
        starts[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][3])
        ends[2] <- as.numeric(strsplit(., split=":", fixed=TRUE)[[1]][2])
        
        widths[2] <- (starts[2] - ends[2]) + 1
    ...
    # Specify per-base coordinates
        ...
        # 5' Alt. exon
        . <- strsplit(tran_id, split=":-@", fixed=TRUE)[[1]][1]
        start <- as.numeric(strsplit(., split="|", fixed=TRUE)[[1]][2]) - 1
    ...
    df$exon.type <- factor(df$exon.type,
                           levels=c("5' Cons. exon", "5' Alt. exon", "3' Cons. exon")
                           )
    ...
    # Define column and colors
        ...
        names(annotation.col.color) <- c("5' Cons. exon", "5' Alt. exon", "3' Cons. exon")
