#' @title Percent spliced-in (PSI) visualization for alternative 5' splice sites (A5SS)
#'
#' @description
#' \code{PlotPSI.A5SS} visualizes percent spliced-in (PSI) for each genomic coordinate for alternative 5' splice site (A5SS) event across groups of single cells.
#'
#' @details
...
#' PlotPSI.A5SS.MultiGroups(PSI, SampleInfo=system.file("extdata/Sample_Info",
...
#'   statistical.test="KW",  multiple.testing="none",
...
PlotPSI.A5SS.MultiGroups <- function(object, SampleInfo, ExonInfo, statistical.test=c("KW", "ANOVA"), multiple.testing=c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"), Plots, plot.width, plot.height) {
    ...
    exon.info <- exon.info[which(exon.info$V2=="A5SS"), ]
    ...
    for(k in 1:nrow(exon.info)) {
        ...
        if(length(strand) != 0) {
            ...
            start.5.constitutive <- as.numeric(strsplit(exon.5.constitutive, split="\\:")[[1]][2])
            
            exon.5.constitutive <- exons.split[1]
            exon.5.constitutive <- strsplit(exon.5.constitutive, split="\\:")[[1]][3]
            end.5.constitutive <- as.numeric(strsplit(exon.5.constitutive, split="\\|")[[1]][1])
            
            # Retrieve AS exon
            exon.AS <- exons.split[1]
            exon.AS <- strsplit(exon.AS, split="\\:")[[1]][3]
            start.AS <- as.numeric(strsplit(exon.AS, split="\\|")[[1]][1]) + 1
            
            exon.AS <- exons.split[1]
            end.AS <- as.numeric(strsplit(exon.AS, split="\\|")[[1]][2])
            ...
            # Retrieve coordinates
                ...
                # Merge
                psi.small <- rbind.data.frame(psi.small.exon.AS, psi.small.exon.5.constitutive)
            ...
            exon.5.constitutive <- exons.split[1]
            exon.5.constitutive <- strsplit(exon.5.constitutive, split="\\:")[[1]][3]
            end.5.constitutive <- as.numeric(strsplit(exon.5.constitutive, split="\\|")[[1]][2])
            
            # Retrieve AS exon
            exon.AS <- exons.split[1]
            exon.AS <- strsplit(exon.AS, split="\\:")[[1]][3]
            start.AS <- as.numeric(strsplit(exon.AS, split="\\|")[[1]][2]) - 1
            
            exon.AS <- exons.split[1]
            exon.AS <- strsplit(exon.AS, split="\\:")[[1]][3]
            end.AS <- as.numeric(strsplit(exon.AS, split="\\|")[[1]][1])
            ...
            # Retrieve coordinates
                ...
                # Merge
                psi.small <- rbind.data.frame(psi.small.exon.5.constitutive, psi.small.exon.AS)
        ...
        # PSI (individual) graph
            ...
            # Define column labels
                ...
                exon.AS <- rep("5' Alt. exon", times=(abs(end.AS-start.AS)+1))
                annotation.col.lab <- data.frame("Base position"=c(exon.5.constitutive, exon.AS))
            ...
            # Define column and colors
                # Columnm color
                annotation.col.color <- c("black", "orange")
                names(annotation.col.color) <- c("5' Cons. exon", "5' Alt. exon")
